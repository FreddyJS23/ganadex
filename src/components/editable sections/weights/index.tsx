...
import {
  weightsShema,
  weightsWith2yearShema,
} from "@/validations/weightsShema";
...
export const WeightsEditable = ({
  ...
  const {
    ...
    resolver: zodResolver(
      typeCattle == "Becerro" ? weightsShema : weightsWith2yearShema,
    ),
    defaultValues:
      typeCattle == "Becerro" ? defaultValues : defaultValuesWith2year,
  });
  ...
  return (
    <>
      ...
      {editable ? (
        ...
        >
          <>
            {formWeights.map(({ id, label, required, type, endContent }) => {
              return (
                !(typeCattle == "Becerro" && id == "peso_2year") && (
                  <div className="w-44" key={id}>
                    <Input
                      ...
                      label={label}
                      type={type}
                      defaultValue={convertWeightToNumber(pesos[id]).toString()}

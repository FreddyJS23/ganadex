import {
  optionsSidebarAdmin,
  optionsSidebarVeterinary,
} from "@/collections/sidebarElements";
...
const sidebarElements = (
  ...
  if (role == "admin")
    return optionsSidebarAdmin.map(({ element, icon, url, options }, key) => (
      <SidebarElement
        key={key}
        element={element}
        icon={icon}
  ...
  if (role == "veterinario")
    return optionsSidebarVeterinary.map(
      ({ element, icon, url, options }, key) => (
        <SidebarElement
          key={key}
          element={element}
          icon={icon}

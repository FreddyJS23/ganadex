...
import {  PreguntasSeguridad, ResponseErrorNext, ResponseRegistroExitoso } from '@/types';
...
export const NewPasswordRecovery = ({
    ...
    const actionRecoveryPassword: () => void = handleSubmit(async (data) => {
        ...
        const response =await fetch(`/api/restablecer_acceso/${token}`,{method:'POST',body:JSON.stringify(sendData)})
        
        const dataResponse:ResponseErrorNext | ResponseRegistroExitoso = await response.json()

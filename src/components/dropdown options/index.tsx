  ...
  if (optionType == "cattle") {
    optionsDropdown =
      role == "admin"
        ? optionsDropdownCattleAdmin
        : optionsDropdownCattleVeterinary;
    if (disabledDiscardCattle)
      optionsDropdown = optionsDropdown.filter(
        (option) => option.url != "descartar_ganado",
      );
        ...
        {({ url, label }) => (
          <DropdownItem key={url + label}>
            ...
            url == "venta_ganado/vender" ? (
              <Link className="block w-full" href={`/${url}/${idCattle}`}>
                {label}
              </Link>
            ) : (
              <Link

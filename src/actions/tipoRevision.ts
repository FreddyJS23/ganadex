...
export async function createTypeCheck(
    formData: CreateTypeCheck,
       ...
       const response= await getData<CreateTypeCheck,ResponseTipoRevision>('tipoRevision', 'POST', formData);
...
export async function updateTypeCheck(
    ...
    id: number,
       ...
       const response= await getData<CreateTypeCheck,ResponseTipoRevision>('tipoRevision', 'PUT', formData, id);
        if(typeof response == 'object' && 'error' in response)  return response
        else return response
}

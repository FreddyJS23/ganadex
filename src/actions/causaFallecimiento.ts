...
export async function createCausaFallecimiento(
    formData: CreateCausaFallecimiento,
       ...
       const response= await getData<CreateCausaFallecimiento,ResponseCausaFallecimiento>('causasFallecimiento', 'POST', formData);
...
export async function updateCausaFallecimiento(
    ...
    id: number,
       ...
       const response= await getData<CreateCausaFallecimiento,ResponseCausaFallecimiento>('causaFallecimiento', 'PUT', formData, id);
        if(typeof response == 'object' && 'error' in response)  return response
        else return response
}

...
export default async function Page() {
  ...
  const response2 = await getData<ResponseSugerirNumero>({
    endPoint: "sugerirNumero",
  });
  const { numero_disponible } = responseErrorServer(response2);

  const response3 = await getData<ResponseVacunasDisponibles>({
    endPoint: "vacunasDisponibles",
  });
  const { vacunas_disponibles } = responseErrorServer(response3);

  const response4 = await getData<ResponseCausasFallecimiento>({
